...
func NewTextButton(res *uiResources, text string, onclick func()) *widget.Button {
	ff := res.fonts.face
	options := []widget.ButtonOpt{
		...
		widget.ButtonOpts.Text(text, ff, res.button.text),
...
func NewButton(res *uiResources, text string, onclick func()) *widget.Button {
...
func NewButtonWithConfig(res *uiResources, config ButtonConfig) *widget.Button {
	ff := config.Font
	if ff == nil {
		ff = res.fonts.face
	}
